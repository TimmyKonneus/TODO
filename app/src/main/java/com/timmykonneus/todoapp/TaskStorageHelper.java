...
public final class TaskStorageHelper implements Handler.Callback {
    ...
    private void internalLoadTasks(final Callback callback) {
        ...
        while (cursor.moveToNext()){
            ...
            long timestamp = cursor.getLong(cursor.getColumnIndex("createdDate"));
            task.setCreatedDate(new Date(timestamp));
    ...
    private void internalSaveTask(Task task) {
        ...
        values.put("createdDate", task.getCreatedDate().getTime());
    ...
    private static class DbSchema {
        ...
        public static final String[] COLUMNS = {"_id", "title", "description", "createdDate", "completed", "archived"};
